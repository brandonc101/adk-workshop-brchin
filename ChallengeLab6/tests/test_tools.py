...
try:  # pragma: no cover - exercised implicitly by the environment
    from readynow_agent.tools import geocode_place, get_directions, get_weather
    ...
    geocode_place = _mod.geocode_place
    get_weather = _mod.get_weather
    get_directions = _mod.get_directions
...
# A reusable, well-formed geocoding success payload (US location).
_GEOCODE_OK = {
    "status": "OK",
    "results": [
        {
...
class ContractTests(unittest.TestCase):
    ...
    def test_success_dict_shape(self, mock_get):
        ...
        self.assertEqual(
            set(result),
            {"status", "latitude", "longitude", "formatted_address", "country"},
        )

    @patch.dict(os.environ, {"GOOGLE_MAPS_API_KEY": "test-key"})
    @patch("requests.get")
    def test_country_empty_when_no_components(self, mock_get):
        # A result with no address_components -> country falls back to "".
        mock_get.return_value = _fake_response(
            {
                "status": "OK",
                "results": [
        ...
        result = geocode_place("Somewhere")
...
class GetDirectionsTests(unittest.TestCase):
    _DIRECTIONS_OK = {
        "status": "OK",
        "routes": [
            {
                "legs": [
                    {
                        "start_address": "Sacramento, CA, USA",
                        "end_address": "UC Davis Medical Center, Sacramento, CA",
                        "distance": {"text": "3.2 mi"},
                        "duration": {"text": "11 mins"},
                        "steps": [
                            {"html_instructions": "Head <b>north</b> on 5th St"},
                            {"html_instructions": "Turn <b>right</b> onto X St"},
                        ],
                    }
                ]
            }
    ...
    @patch.dict(os.environ, {"GOOGLE_MAPS_API_KEY": "test-key"})
    @patch("requests.get")
    def test_success_strips_html(self, mock_get):
        mock_get.return_value = _fake_response(self._DIRECTIONS_OK)

        result = get_directions("Sacramento, CA", "UC Davis Medical Center")

        self.assertEqual(result["status"], "success")
        self.assertEqual(result["distance"], "3.2 mi")
        self.assertEqual(result["duration"], "11 mins")
        self.assertEqual(result["steps"], ["Head north on 5th St", "Turn right onto X St"])

    @patch.dict(os.environ, {"GOOGLE_MAPS_API_KEY": ""})
    @patch("requests.get")
    def test_missing_api_key(self, mock_get):
        result = get_directions("A", "B")
        self.assertEqual(result["status"], "error")
        self.assertIn("GOOGLE_MAPS_API_KEY", result["error_message"])
    ...
    def test_no_route_found(self, mock_get):
        mock_get.return_value = _fake_response({"status": "ZERO_RESULTS", "routes": []})
        result = get_directions("A", "B")
        self.assertEqual(result["status"], "error")
        self.assertIn("ZERO_RESULTS", result["error_message"])
    ...
    def test_network_error(self, mock_get):
        ...
        result = get_directions("A", "B")
        self.assertEqual(result["status"], "error")
        self.assertIn("Directions API", result["error_message"])
