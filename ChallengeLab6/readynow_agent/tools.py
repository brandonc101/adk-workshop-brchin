...
def _extract_country_code(address_components: list) -> str:
    """Return the ISO 3166-1 alpha-2 country code from geocoding components.

    Args:
        address_components: The ``address_components`` list from a Google
            Geocoding API result.

    Returns:
        The two-letter country code (e.g. ``"US"``), or "" if not present.
    """
...
def geocode_place(place: str) -> dict:
    ...
    return {
        ...
        "country": _extract_country_code(top_result.get("address_components", [])),
    }
...
def _strip_html(text: str) -> str:
    """Remove HTML tags from a string (Directions steps use HTML markup)."""
    return re.sub(r"<[^>]+>", "", text or "").strip()


def get_directions(origin: str, destination: str) -> dict:
    """Get driving directions between two places (routes to safety).

    Uses the Google Maps Directions API to produce turn-by-turn driving
    directions - for example, to route a user away from a disaster area toward
    a shelter, hospital, or other safe location. The API key is read from the
    ``GOOGLE_MAPS_API_KEY`` environment variable.

    Args:
        origin: The starting place name or address (e.g. "Sacramento, CA").
        destination: The destination place name or address (e.g. a shelter or
    ...
    Returns:
        A dictionary describing the result. On success it contains::

            {
                ...
                "start_address": <str>,
                "end_address": <str>,
                "distance": <str>,        # e.g. "12.4 mi"
                "duration": <str>,        # e.g. "18 mins"
                "steps": [<str>, ...],    # turn-by-turn instructions
            }
    ...
    try:
        ...
        return {
            "status": "error",
            "error_message": f"Could not reach the Directions API: {exc}",
        }
    ...
    if payload.get("status") != "OK" or not payload.get("routes"):
        ...
        return {
            "status": "error",
            "error_message": (
                f"Directions failed from '{origin}' to '{destination}': "
                f"{payload.get('status', 'UNKNOWN')}"
                + (f" - {detail}" if detail else "")
            ),
        }
    ...
    return {
        ...
        "steps": [_strip_html(step.get("html_instructions", "")) for step in leg["steps"]],
    }
