...
def _block(reason: str, text: str) -> LlmResponse:
    """Log a blocked prompt and build the refusal LlmResponse."""
    logger.warning("[BLOCKED] reason=%s prompt=%r", reason, text)
    return LlmResponse(
        content=types.Content(
            role="model",
...
def screen_input_safety(
    callback_context: CallbackContext,
    llm_request: LlmRequest,
) -> Optional[LlmResponse]:
    """Before-model callback: block malicious input via Model Armor.

    Used on the root/coordinator agent so every request is safety-screened
    regardless of which sub-agent will handle it.
    ...
    """
    text = extract_latest_user_text(llm_request.contents)
    is_safe, reason = screen_user_prompt(text)
    if is_safe:
        return None
    return _block(reason, text)
...
def validate_us_location(
    ...
    """Before-model callback: block non-US locations.

    Used on the weather agent, whose National Weather Service tools only cover
    the United States. General (non-weather) requests routed to other agents
    ...
    """
    text = extract_latest_user_text(llm_request.contents)
    allowed, reason = us_location_check(text)
    if allowed:
        return None
    return _block(reason, text)
